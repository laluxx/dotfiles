...
const int[16] dither_table_4x4 = int[](
	0,8,2,10,
	12,4,14,6,
	3,11,1,9,
	15,7,13,5
...
float dither_4x4(vec2 coord, float color){
	// get matrix position
	int x = int(mod(coord.x, 8.0)/2);
	int y = int(mod(coord.y, 8.0)/2);
	int index = x + 4*y;
	float limit = (float(dither_table_4x4[index]) + 1.0) / 16.;
	
	float diff;
	// clamp down color and save diff 0-1 value
	diff = modf(color, color);
	if (diff > limit)
		color += 1.0;
	return color;
}
...
vec4 window_shader(){
	...
	color.xyz = rgb_to_hsv(color.rgb * opacity);
	if (invert_color)
		color.z = 1.0 - color.z;
	color.yz = dither(texcoord, color.gb);
	color.a *= dither_4x4(texcoord, opacity);
