...
declare -A image_map=(
  ...
  ["haskell"]="$HOME/Desktop/xos/xicons/haskell.png"
  ["python"]="$HOME/Desktop/xos/xicons/python.png"
  ["test"]="$HOME/Desktop/xos/xicons/test.png"
...
function display_random_image() {
    path=~/Desktop/xos/xicons
    files=($path/*)
    rand=$[$RANDOM % ${#files[@]}]
    image_path=${files[$rand]}
...
#old chpwd
# Call the display_image function when you cd into a directory
#function chpwd() {
...
# lliterally see a generated image of the current directory using kitty or wezterm
function kls() {
  exa -1 --color=auto | awk '{print length, $0}' | sort -rn | awk '{print $2}' | awk '
    BEGIN {
    ...
    }
    {
     ...
     icon = icons[$0]
      if (!icon) {
        icon = "" # Default icon for other directories
      }

      # Print the output with the icon
      printf("%s %s\n", icon, $0)
    }' | sed "s/\./DOT/g;s/\//🐾/g" | awk -v term_cols="$(tput cols)" '
      {
        # Calculate the amount of padding needed to center the text
        padding = int((term_cols - length($0)) / 2)
        printf "%*s%s\n", padding, "", $0

      }' | convert -background none -fill "#95CBF9" -font "/usr/share/fonts/JetBrainsMono/JetBrainsMono-Bold.ttf" -pointsize 30 -gravity center label:@- -trim +repage output.png && kitty +kitten icat output.png
}
...
function gup() {
  ...
  git_commit_message="Add new files ($(date +'%Y-%m-%d %H:%M:%S'))"
...
function gclone() {
  ...
  kitty +kitten icat ~/Desktop/xos/xicons/git.png
...
#HACK
function pkgwrite() {
  # Get list of installed packages and write to file
  pacman -Qqe > "$(pwd)/$1"

  # Print success message
  echo "List of installed packages written to $(pwd)/$1"

  # Display file using less
...
# function isolate {
#   if [[ $# -eq 0 ]]; then
#     echo "Usage: isolate <file/directory>"
#     return 1
#   fi
