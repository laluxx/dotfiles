...
#+begin_src emacs-lisp
;; TODO
;; this worked one time after i doom synced
;; after an emacs restart it doesn't work anymore
(defvar my-original-mode-line-format nil
  "Backup of the original mode-line-format.")

(defvar my-spaceline-mode-line-format nil
  "Backup of the spaceline mode-line-format.")

;; Backup the original doom modeline (do this after doom has initialized)
(add-hook 'doom-post-init-modules-hook
          (lambda ()
            (setq my-original-mode-line-format mode-line-format)))

;; Backup spaceline format (do this after spaceline has initialized)
(with-eval-after-load 'spaceline
  (spaceline-spacemacs-theme)
  (setq my-spaceline-mode-line-format mode-line-format))


(defun switch-to-doom-modeline ()
  (interactive)
  (when my-original-mode-line-format
    (setq mode-line-format my-original-mode-line-format))
  (doom-modeline-mode 1)
  (force-mode-line-update t))

(defun switch-to-spaceline ()
  (interactive)
  (doom-modeline-mode -1)
  (when my-spaceline-mode-line-format
    (setq mode-line-format my-spaceline-mode-line-format))
  (force-mode-line-update t))
#+end_src
...
#+begin_src emacs-lisp
(map! :leader
      "fi" #'swiper-isearch) ; or whatever the precise name of the function you want to bind to is.
#+end_src
** INSERT
#+begin_src emacs-lisp
(after! all-the-icons
  (map! :leader
        (:prefix ("i" . "insert")
         :desc "Insert icon" "i" #'all-the-icons-insert)))

  (map! :leader
        (:prefix ("i" . "insert")
         :desc "insert file icon" "f" #'all-the-icons-insert-fileicon))

  (map! :leader
        (:prefix ("i" . "insert")
         :desc "insert table" "t" #'table-insert))
#+end_src
** OPEN
*** Ielm
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
;; TODO
;; (defun my/select-or-create-workspace (index)
;;   "Switch to the workspace INDEX or create it if it doesn't exist."
;;   (let ((name (number-to-string (1+ index))))
...
;; (dotimes (i 9)
;;   (map! :leader
;;         :n (vector 'M- (int-to-string (1+ i)))
;;         (lambda () (interactive)
;;           (my/select-or-create-workspace i))))

(defun custom/switch-or-create-workspace (index)
  "Switch to workspace INDEX. If it doesn't exist, create it and then switch."
  (let ((name (number-to-string index)))
    (unless (+workspace-exists-p name)
      (while (not (+workspace-exists-p name))
        (+workspace-new (number-to-string (+ 1 (length (+workspace-list-names))))))
      (dashboard-open))
    (funcall (intern (format "+workspace/switch-to-%d" index)))))

;; Define keybindings for Alt + 1 to Alt + 9
(dotimes (i 9)
  (map! :g (kbd (format "M-%d" (+ i 1)))
        (lambda () (interactive) (custom/switch-or-create-workspace i))))
...
#+end_src
** Delete-selection-and-count
#+begin_src emacs-lisp
;; (defun delete-and-count ()
;;   "Delete a character and display the count of characters and words deleted in the minibuffer."
;;   (interactive)
;;   (unless (eobp) ;; Don't try to delete if at the end of buffer
;;     (let* ((beg (point))
;;            (end (1+ beg))
;;            (content (buffer-substring beg end))
;;            (char-count (length content))
;;            (word-count (length (split-string content "\\W+" t))))
;;       (delete-region beg end)
;;       (message "Deleted %d chars, %d words" char-count word-count))))

;; (after! evil
;;   (map! :n "x" #'delete-and-count))
#+end_src
** Dynamic-modeline
#+begin_src emacs-lisp
;; V1
...
(defun normalize-color (color)
  "Normalize COLOR values to 0-255 RGB."
  (mapcar (lambda (x) (/ x 256)) color))
...
(defun counter-color (value)
  ...
  (let* ((success-color (normalize-color (color-values (face-foreground 'success))))
         (error-color (normalize-color (color-values (face-foreground 'error))))
         (color (interpolate-color value 100 success-color error-color)))
...
(after! hl-line
  ...
  (defun laluxx/toggle-local-hl-line ()
    (interactive)
    (if hl-line-mode
        (hl-line-mode -1)
      (hl-line-mode 1)))
  ...
  (defun laluxx/toggle-global-hl-line ()
    (interactive)
...
(map! :leader
      (:prefix ("t" . "toggle")
       :desc "Toggle hl-line in current buffer" "h" #'laluxx/toggle-local-hl-line
       :desc "Toggle global hl-line" "H" #'laluxx/toggle-global-hl-line))
#+end_src
** Wal-Set
*** Default
TODO:
...
#+begin_src emacs-lisp
(after! good-scroll
  (map! :g [next]  #'good-scroll-up-full-screen
        :g [prior] #'good-scroll-down-full-screen))
#+end_src
** Org
#+begin_src emacs-lisp
...
*** Roam
#+begin_src emacs-lisp
(setq org-directory "~/org/")  ; Set the main directory for org files
(setq org-default-notes-file (concat org-directory "inbox.org"))  ; Default file for new notes
(setq org-agenda-files '("~/org/"))  ; Directory for agenda files
#+end_src
** Aggressive indent
Activate for lisp-like languages
#+begin_src emacs-lisp

;; Activate aggressive-indent-mode only for lisp-like modes
...
;; Enable color-identifiers-mode globally
(after! color-identifiers-mode

  ;; Adjust recoloring delay
  ...
  ;; Regenerate colors after theme change
  (defun my-ewal-theme-hook ()
    (when (eq doom-theme 'ewal-doom-one)
      (color-identifiers:regenerate-colors)))
  (add-hook 'doom-load-theme-hook 'my-ewal-theme-hook)

  ;; Toggle mode to ensure changes are applied
...
#+begin_src emacs-lisp
(after! dimmer
  ;; Set the adjustment mode and fraction
  (setq dimmer-adjustment-mode :foreground) ;; Choices: :foreground, :background, :both
  (setq dimmer-fraction 0.30) ;; Increase if you'd like other buffers to be more dim.

  ;; Exclude certain buffers from dimming
  (setq dimmer-buffer-exclusion-regexps '("^\\*Minibuf.*\\*"))

  ;; Exclude buffers using predicate functions
  ;; Example: don't dim buffers that are read-only
  (add-to-list 'dimmer-buffer-exclusion-predicates
               (lambda (buf) (with-current-buffer buf buffer-read-only)))

  ;; Dim when Emacs loses focus
  (setq dimmer-watch-frame-focus-events t)

  ;; Ensure certain popups and modes do not get dimmed
  (dimmer-configure-which-key)
  (dimmer-configure-helm)
  ;; Add other modes you use, for example:
