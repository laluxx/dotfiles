...
#+end_src
*** C-to-org
dont really work, maybe i will do bash to org
#+begin_src shell
(defun laluxx/c-to-org ()
  (interactive)
  (let* ((buffer-contents (buffer-string))
         (org-buffer (get-buffer-create (concat (file-name-base (buffer-name)) ".org")))
         (includes (s-match-strings-all "^#include .*" buffer-contents))
         (functions (s-match-strings-all "^\\(.*\\)\\(\\w+ \\*?\\w+\\)(.*)\n{" buffer-contents))
         (main (s-match "^int main.*" buffer-contents)))
    (with-current-buffer org-buffer
      (insert "* Imports\n")
      (insert "#+BEGIN_SRC c\n")
      (mapc (lambda (include) (insert (car include) "\n")) includes)
      (insert "#+END_SRC\n")
      (insert "* Functions\n")
      (mapc (lambda (function)
              (insert (format "* %s\n" (second (split-string (cadr function))))))
            functions)
      (when main
        (insert "* Main\n")
        (insert "#+BEGIN_SRC c\n")
