...
I am maintaining this config not just for myself, but also for those that want to explore some of what is possible with Emacs. I will add a lot of examples of plugins and settings.

* TODO
** org
understand lsp work inside org documents too
* XOS
** update-dotfiles
...
;;0.0.0
;; (defun vterm-toggle-cd-lfcd ()
;;   "Open a vterm buffer in the bottom window running zsh and execute 'lfcd'."
;;   (interactive)
;;   (let ((buffer-name "lfcd-vterm")
...
;;         (other-window 1)
;;         (vterm buffer-name)
;;         (vterm-send-string "lfcd")
;;         (vterm-send-return)))))

;; (map! :leader
;;       (:prefix ("d" . "custom")
;;        :desc "Open vterm with lfcd" "l" #'vterm-toggle-cd-lfcd))
#+end_src
**** perosnal config
#+begin_src emacs-lisp
(map! :leader
...
**** delete-empty-pair
#+begin_src emacs-lisp
(defun laluxx/delete-empty-pair ()
  (interactive)
...
(global-set-key (kbd "<backspace>") 'laluxx/delete-empty-pair)
#+end_src
**** wrap
#+begin_src emacs-lisp
;; (defun wrap (delimiter)
;;   "Wrap region with DELIMITER if region is active."
;;   (interactive "cEnter a character to wrap with: ")
;;   (if (region-active-p)
