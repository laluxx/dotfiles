...
#+begin_src emacs-lisp
;; IF only one window
;; ctrl h and l will cycle buffers
;; (defun previous-buffer-or-window-left ()
;;   "Switch to previous buffer if there's only one window. Otherwise, enlarge the window to the left."
;;   (interactive)
;;   (if (one-window-p)
;;       (previous-buffer)
;;     (resize-window-horizontally nil)))

;; (defun next-buffer-or-window-right ()
;;   "Switch to next buffer if there's only one window. Otherwise, enlarge the window to the right."
;;   (interactive)
;;   (if (one-window-p)
;;       (next-buffer)
;;     (resize-window-horizontally t)))

;; (global-set-key (kbd "C-h") 'previous-buffer-or-window-left)
;; (global-set-key (kbd "C-l") 'next-buffer-or-window-right)
#+end_src
...
;; (global-set-key (kbd "C-h") (lambda () (interactive) (resize-window-horizontally nil)))
;; (global-set-key (kbd "C-l") (lambda () (interactive) (resize-window-horizontally t)))
#+end_src
*** Resizing and Buffer Cycling
ctrl + h -> enlarge left  [x]
ctrl + l -> enlarge right [x]
- if there is only one window ctrl h and l wii switch buffer [x]
...
#+begin_src emacs-lisp
;; Keybindings
(map! :n "C-M-l" #'next-buffer
      :n "C-M-h" #'previous-buffer
      "C-j" #'other-window
