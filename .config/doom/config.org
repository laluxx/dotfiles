#+TITLE: XOS Doom Bloated Emacs Config
#+AUTHOR: laluxx
...
- [[#ewal][Ewal]]
  - [[#ewal-doom-themes][Ewal-Doom-Themes]]
  - [[#ewal-evil-cursor][Ewal-Evil-Cursor]]
  - [[#spaceline][Spaceline]]
- [[#gptel][Gptel]]
- [[#keybinds][Keybinds]]
  - [[#dmenu][Dmenu]]
  - [[#help-hooks][Help-hooks]]
  - [[#movement][movement]]
  - [[#toggle][toggle]]
  - [[#resize-window][resize-window]]
  - [[#look-and-feel][look and feel]]
- [[#org][org]]
  - [[#smart-delimiters][smart-delimiters]]
  - [[#insert][INSERT]]
- [[#beacon][BEACON]]
...
(defun laluxx/wal-set ()
  ...
  (let* ((default-directory "~/xos/wallpapers/static")
         ...
         (counsel-find-file (counsel--find-return-list
                             (lambda (x)
                               (let ((ext (file-name-extension x)))
                                 (and ext (member ext '("png" "jpg" "jpeg" "webp"))))))))
    (ivy-read "Choose wallpaper: "
              counsel-find-file
              :action (lambda (wallpaper)
                        (when (and (not (string-empty-p wallpaper))
                                   (file-exists-p wallpaper))
                          (let ((abs-wallpaper (expand-file-name wallpaper default-directory)))
...
(defun laluxx-doom-wal ()
  ...
  (shell-command "python3 /home/l/xos/pywal-scripts/xmonad-dark-wal.py")    ;; dark  theme
...
(setq my:rice:font (font-spec :family "Source Code Pro" :weight 'semi-bold :size 11.0))
...
#+begin_src emacs-lisp
(map! :leader
      :desc "Open test directory"
      "f t" (lambda () (interactive) (dired "~/Desktop/test")))
#+end_src
*** xos
#+begin_src emacs-lisp
(map! :leader
      :desc "Open xos directory"
      "f x" (lambda () (interactive) (dired "~/xos")))
#+end_src
*** Zsh
#+begin_src emacs-lisp
(map! :leader
      :desc "Open dotfiles zsh directory"
      "f z" (lambda () (interactive) (dired "~/Desktop/pulls/dotfiles/.config/zsh")))
#+end_src
** toggle
*** treemacs
#+begin_src emacs-lisp
...
*** update-and-refresh
#+begin_src emacs-lisp
;; (defun laluxx/update-org-colors-and-refresh ()
;;   "Reload the config file and refresh the color scheme."
;;   (interactive)
;;   ;; Load the file
;;   (load-file (expand-file-name "~/.config/doom/config.org"))
;;   ;; Then call your function to refresh the color scheme
;;   (laluxx/org-colors-wal))
#+end_src
*** detect-double-asterisk
*DONT WORK*
#+begin_src emacs-lisp
;; (defun laluxx/detect-double-asterisk ()
;;   "If the last two characters typed are '**', move cursor in between."
;;   (message "Running laluxx/detect-double-asterisk")
...
#+begin_src emacs-lisp
(defun org-remove-duplicate-headings ()
  "Remove duplicate org headings and keep the last one."
  (interactive)
  (save-excursion
    (goto-char (point-min))
    (let ((headings (make-hash-table :test 'equal))
      ...
      (while (re-search-forward org-heading-regexp nil t)
        (let ((heading (match-string-no-properties 0)))
          (if (gethash heading headings)
              (push (cons (copy-marker (match-beginning 0)) heading) dups)
            (puthash heading t headings))))
      (dolist (dup (nreverse dups))
        (delete-region (marker-position (car dup))
...
#+begin_src emacs-lisp
;; TODO
;; (defface my-variable-face
;;   '((t :family "OCR B" :height 120))  ;; Adjust size as needed
;;   "My face for variable identifiers.")

;; (defun my-variable-highlighter (limit)
;;   (while (re-search-forward "\\_<[a-z_][a-zA-Z0-9_]*\\_>" limit t)
;;     (let ((face (get-text-property (match-beginning 0) 'face)))
;;       (when (and (listp face) (member 'font-lock-variable-name-face face))
;;         (put-text-property (match-beginning 0) (match-end 0) 'face 'my-variable-face)))))

;; (defun my-variable-hook ()
;;   (font-lock-add-keywords nil '((my-variable-highlighter)) 'append))

;; (add-hook 'emacs-lisp-mode-hook 'my-variable-hook)
#+end_src
...
*TODO:* make org-colors-wal work make a python script

#+begin_src emacs-lisp
(defun laluxx/org-colors-wal ()
  "Enable wal colors for Org headers."
  (interactive)
  (dolist
      (face
