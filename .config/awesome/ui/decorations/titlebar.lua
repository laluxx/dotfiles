...
-- 0.0.0
local function create_sticky_indicator(c)
    ...
    local function update_sticky_indicator()
      if c.sticky then
        indicator.bg = "#6791C9"
      else
        indicator.bg = "#0C0E0F"
    ...
    c:connect_signal("property::sticky", update_sticky_indicator)
    update_sticky_indicator()
...
client.connect_signal("request::titlebars", function(c)
	...
	awful
		...
		:setup({
			...
			widget = wibox.container.background,
      create_sticky_indicator(c), -- IF YOU WANT PIN TO BE AT BOTTOM
		})
end)
