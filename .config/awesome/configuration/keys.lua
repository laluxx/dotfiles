...
-- SPAWN KITTY WITH GLASS CONF
-- awful.key({ mod }, "Return", function()
--     awful.spawn("kitty --config /home/l/Desktop/xos/modules/term/kitty/trasparent/kitty.conf")
-- end, { description = "Open Kitty Terminal", group = "launcher" }),


--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

--TERMINAL BASED ON THE CURRENT TAG

-- awful.key({ mod }, "Return", function()
--     local tag_index = awful.screen.focused().selected_tag.index
--     local terminal_cmd

--     if tag_index == 1 then
--         terminal_cmd = "kitty --config /path/to/first/config.conf"
--     elseif tag_index == 2 then
--         terminal_cmd = "kitty --config /path/to/second/config.conf"
--     elseif tag_index == 3 then
--         terminal_cmd = "kitty --config "
--     else
--         terminal_cmd = "kitty --config /path/to/default/config.conf"
--     end
...
awful.key({ mod }, "Return", function()
    ...
    if tag_index == 1 then
        awful.spawn("st")
        -- awful.spawn("xdotool key super+h c Enter super+l")
    elseif tag_index == 2 then
        awful.spawn("kitty --config /home/l/Desktop/xos/modules/term/kitty/trasparent/kitty.conf")
    elseif tag_index == 3 then
        awful.spawn("st")
    elseif tag_index == 4 then
        awful.spawn("st")
    elseif tag_index == 5 then
        awful.spawn("st")
    elseif tag_index == 6 then
        awful.spawn("st")
    -- elseif tag_index == 2 then
    --     awful.spawn("kitty --config /home/l/Desktop/xos/modules/term/kitty/trasparent/kitty.conf")
    -- elseif tag_index == 3 then
    --     awful.spawn(apps.default.floating_kitty_xonsh, { sticky = true })
    --         local callback = function(c)
    --             c.floating = true
    --             c.ontop = true
    --             c:geometry({ width = 800, height = 600, x = 1000, y = 250 })
    --         end
    ...
    --             if c.class == "floating-kitty-xonsh" then
    --               callback(c) awful.spawn("xdotool key super+2")
    --             end
    -- end)
...
--         awful.spawn(apps.default.floating_kitty_xonsh, {
--                       sticky = true
-- })
    else
        awful.spawn(apps.default.floating_kitty_xonsh, {
...
awful.key({ mod, "Shift" }, "Return", function()
...
end, { description = "open floating kitty with xonsh", group = "app" }),
    ...
    --     c.floating = true
    --     c.ontop = true
    --     c:geometry({ width = 509, height = 1058, x = -4, y = 22 })
    -- end
    -- for _, c in ipairs(client.get()) do
    --     if c.class == "floating-kitty-xonsh" then
    --         callback(c)
...
awful.key({ mod }, "a", function()
    awful.spawn.with_shell(apps.default.toggle_picom_animations)
end, { description = "toggle picom animations", group = "awesome" }),
...
awful.key({ mod, "Control" }, "s", function()
    ...
    if c then
        c:relative_move(0, 0, c.width * -0.25, c.height * -0.25)
    end
end, { description = "riduci la finestra del 75%", group = "finestra" }),


awful.key({ mod, "Control" }, "d", function()
    local c = client.focus
    if c then
        c:relative_move(0, 0, c.width * 0.25, c.height * 0.25)
    end
end, { description = "aumenta la dimensione della finestra del 25%", group = "finestra" }),
...
awful.key({ mod }, "s", function()
    local c = client.focus
    if c then
        local new_width = c.width * 0.75
        local new_height = c.height * 0.75
        local x_offset = (c.width - new_width) / 2
        local y_offset = (c.height - new_height) / 2
        c:geometry({ width = new_width, height = new_height, x = c.x + x_offset, y = c.y + y_offset })
    end
end, { description = "riduci la finestra del 25%", group = "finestra" }),
...
awful.key({ mod }, "d", function()
    local c = client.focus
    if c then
        local new_width = c.width * 1.25
        local new_height = c.height * 1.25
        local x_offset = (c.width - new_width) / 2
        local y_offset = (c.height - new_height) / 2
        c:geometry({ width = new_width, height = new_height, x = c.x + x_offset, y = c.y + y_offset })
    end
...
-- Image Displayer in floating Kitty TODO
-- awful.key({ mod, "Shift" }, "i", function()
--     awful.spawn(apps.default.image_displayer)
--     local callback = function(c)
--         c.floating = true
--         c:geometry({ width = 800, height = 600, x = 1000, y = 250 })
--     end
--     for _, c in ipairs(client.get()) do
--         if c.class == "image-displayer" then
--             callback(c)
--         end
--     end
--     client.connect_signal("manage", function(c)
...
--         end
--     end)
-- end, { description = "open floating kitty as image displayer", group = "app" }),


-- Scratch terminal in WezTerm
-- awful.key({ modkey, "Shift" }, "d", function()
--     apps.scratch_terminal()
--     local callback = function(c)
--         c.floating = true
--         c:geometry({ width = 800, height = 600, x = 1000, y = 250 })
--     end
...
--         end
--     end)
-- end, { description = "toggle scratchpad terminal", group = "app" }),
